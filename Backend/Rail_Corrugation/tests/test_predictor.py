...
class PredictorTests(unittest.TestCase):
    ...
    def test_in_memory_analysis_matches_path_analysis(self):
        raw = self.first.read_bytes()
        expected = predictor.analyse_file(self.first)
        actual = predictor.analyse_bytes(raw, self.first.name)
        self.assertEqual(actual, expected)

    def test_in_memory_analysis_rejects_bad_uploads(self):
        with self.assertRaisesRegex(ValueError, "CSV"):
            predictor.analyse_bytes(self.first.read_bytes(), "recording.txt")
        with self.assertRaisesRegex(ValueError, "empty"):
            predictor.analyse_bytes(b"", "empty.csv")
        with self.assertRaisesRegex(ValueError, "expected 10,000 rows"):
            predictor.analyse_bytes(b"wrong,header\n1,2\n", "bad.csv")
    ...
    def test_fresh_process_works_without_notebook_or_training_data(self):
        ...
        (isolated / "model").mkdir(parents=True)
        shutil.copy2(PROJECT_DIR / "predictor.py", isolated / "predictor.py")
        shutil.copy2(predictor.DEFAULT_MODEL_PATH, isolated / "model" / "rail_corrugation.joblib")
        code = (
