...
disabled in that original run. Subsequent exploratory hard-case experiments are
reported separately in [REFINEMENT_RESULTS.md](REFINEMENT_RESULTS.md); they did
not replace the deployed model or change the original validation estimate.
...
The app uses a packaged copy at `artifacts/model.json`; see [APP.md](APP.md).
...
All 20 original automated tests passed. Additional checks verified actual saved inner/outer
...
The subsequent app integration passed 38 Python tests, frontend production build
and lint, and a headless-browser check using all 16 test recordings. The actual
browser download matches command-line inference and the original saved
predictions to numerical tolerance. These checks validate integration, not hidden
test accuracy. See [APP.md](APP.md) for run and verification instructions.
