...
def predict(args):
    require(args.model is not None and args.input is not None, "predict requires --model and --input")
    ...
    records = [dict(file_id=p.name, **extract_file(p, args.cache, artifact["observations"])["features"]) for p in files]
    frame = pd.DataFrame(records)
    predictions = FittedModel.from_dict(artifact["model"]).predict(frame)
    require(frame.file_id.is_unique, "Duplicate inference filenames")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(dict(file_id=frame.file_id, prediction=predictions)).to_csv(args.output, index=False)
