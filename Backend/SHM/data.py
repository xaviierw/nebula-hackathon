...
import pandas as pd
import rainflow
from .core.data import OBSERVATIONS, read_raw
from .core.features import (CACHE_VERSION, EXPONENTS, extract_signal, weighted_quantile,
                            logq_column, mean_column)
...
def read_signal(raw, expected_rows=OBSERVATIONS):
    return read_raw(io.BytesIO(raw), expected_rows).iloc[:, 0].to_numpy()
