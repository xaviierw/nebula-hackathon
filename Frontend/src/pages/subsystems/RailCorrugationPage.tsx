...
import { runRailPrediction } from '../../features/rail/runPrediction'
...
import { createEntries } from '../../features/rail/workspace'
...
export function RailCorrugationPage() {
  ...
  async function processEntries(targetBatch: string, entries: RailQueueEntry[]) {
    ...
    try {
      for (const entry of entries) {
        ...
        if (entry.status !== 'queued' || !file) continue
        updateEntry(targetBatch, entry.id, { status: 'analysing', error: null, excluded: false })
        try {
          const analysis = await runRailPrediction(file, request.signal)
          if (request.signal.aborted) break
          updateEntry(targetBatch, entry.id, { status: 'done', analysis, analysedAt: new Date().toISOString() })
          files.current.delete(entry.id)
        } catch (caught) {
          if (request.signal.aborted) break
          updateEntry(targetBatch, entry.id, { status: 'error', error: caught instanceof Error ? caught.message : 'Analysis failed. Please retry.' })
        }
      }
    } finally {
  ...
  function retry(entry: RailQueueEntry, replacement?: File) {
    ...
    if (!file.name.toLowerCase().endsWith('.csv') || file.size > 100 * 1024 * 1024) { setError('Select a CSV of 100 MB or less.'); return }
    setError(null)
